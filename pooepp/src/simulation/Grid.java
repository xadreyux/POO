...
public class Grid {
	...
	public void addSCZone(int c_init, int r_init, int c_end, int r_end, int cost) {
		for (int i = c_init - 1; i <= c_end - 1; i++) {
			if (i != c_init - 1 && i != 0) {
				pGrid[i][r_init - 1].edges[3] = cost;
				pGrid[i][r_end - 1].edges[3] = cost;
			}
			if (i != c_end - 1 && i != ncols - 1) {
				pGrid[i][r_init - 1].edges[1] = cost;
				pGrid[i][r_end - 1].edges[1] = cost;
			}
		}
		for (int j = r_init - 1; j <= r_end - 1; j++) {
			if (j != r_init - 1 && j != 0) {
				pGrid[c_init - 1][j].edges[2] = cost;
				pGrid[c_end - 1][j].edges[2] = cost;
			}
			if (j != r_end - 1 && j != nrows - 1) {
				pGrid[c_init - 1][j].edges[0] = cost;
